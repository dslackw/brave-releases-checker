...
- **Config File Location Clarification:** Added detailed comments to the config.ini example file, explaining the two possible search paths (/etc/brave-releases-checker/config.ini and ~/.config/brave-releases-checker/config.ini) and the loading order. This improves usability and clarity for users setting up or modifying their configuration.
